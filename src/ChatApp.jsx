...
export default function ChatApp() {
        ...
        const setupApp = async () => {
            ...
            if (isConnected && walletClient && address) {
                ...
                if (connectedContract) {
                    ...
                    showPopup('Não foi possível conectar a nenhum RPC da Monad Testnet.', 'error');
                }
            }
        };
        ...
        Object.values(challenges).forEach(c => {
            ...
            if (c.challenger.address.toLowerCase() === address.toLowerCase()) {
                if (c.status === 'accepted' && !activeGame) {
                    ...
                    showPopup(
                        `Desafio aceito por ${c.opponent.username}! Jogo começando...`,
                        'success'
                    );
                    setChallenges(prev => {
                    ...
                    showPopup(`${c.opponent.username} recusou seu desafio.`, 'info');
                    ...
                    showPopup('O oponente recusou o rematch.', 'info');
                    setChallenges(prev => {
                        const newState = { ...prev };
                        if (newState[c.id]) {
    ...
    const initiateConnection = async (walletProvider, walletName) => {
        ...
        try {
            ...
            if (accounts.length === 0) {
                hidePopup();
                showPopup('Sem conta selecionada', 'error');
                return;
            }
            ...
            if (!profileExists) {
                ...
                hidePopup();
                showPopup('Carteira desconectada!', 'success');
            }
        ...
        const initApp = async () => {
            ...
            try {
                ...
                if (error.code === -32007 || (error.error && error.error.code === -32007)) {
                    showPopup('A rede está congestionada. Por favor, tente novamente.', 'error', true);
                }
            } finally {
    ...
    const handleChallengeUser = (opponentAddress, opponentUsername, gameType) => {
        if (!isConnected || !userProfile?.exists) {
            showPopup("Você deve estar conectado e ter um perfil para desafiar alguém.", "error");
            return;
        }
        if (activeGame) {
            showPopup("Você já está em um jogo!", "warning");
            return;
        }
        ...
        showPopup(`Desafio enviado para ${opponentUsername}!`, 'info');
    };
    ...
    const handleAcceptChallenge = (challengeId) => {
        ...
        setIncomingChallenge(null);
        showPopup(`Desafio aceito de ${challengeToAccept.challenger.username}! Jogo começando...`, 'success');
    };
    
    const handleDeclineChallenge = useCallback((challengeId) => {

        setIncomingChallenge(null);
        showPopup("Desafio recusado.", "info");
    ...
    const handleGameEnd = useCallback((sessionId, result) => {
        ...
        if (result === 'closed') {
            showPopup("Jogo fechado", "info");
        } else if (result === 'opponent_left') {
            showPopup("Oponente saiu do jogo. Fim de jogo.", "warning");
        } else if (result === 'rematch_declined') {
            showPopup("O oponente recusou.", "info");
        }
                                        ...
                                        <div className="message-bubble system">
                                            ...
                                            <div className="flex items-center gap-2 mt-2 text-xs justify-center">
                                                <span 
                                                    ...
                                                    title={`${new Date(systemMessage.timestamp * 1000).toLocaleDateString('pt-BR', {
                                                        year: 'numeric',
                                                        month: '2-digit',
                                                        day: '2-digit'
                                                    })} ${new Date(systemMessage.timestamp * 1000).toLocaleTimeString('pt-BR', {
                                                        hour: '2-digit',
                                                        minute: '2-digit',
                                                        hour12: false
                                                    })}`}
                                    ...
                                    <div className={`message ${group.isOwn ? 'own' : 'other'}`}>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                            ...
                                                            if (replyMessage && !replyMessage.excluida) {
                                                                ...
                                                                return (
                                                                    <div className="reply-preview-deleted">
                                                                        ...
                                                                        Resposta a uma mensagem deletada.
                                                                    </div>
                                                                );
                                                            } else {
                                                                return (
                                                                    <div className="reply-preview-deleted" title="a mensagem é antiga, vá para o topo e clique em 'carregar mais'">
                                                                        ...
                                                                        Resposta a uma mensagem que não foi carregada.
                                                                    </div>
                                                                );
                                                            }
                                                        })()}
                                                        ...
                                                        <div className="msg-btnact flex items-center gap-2 mt-2 text-xs">
                                                            <span 
                                                                className="text-gray-400" 
                                                                title={`${new Date(message.timestamp * 1000).toLocaleDateString('pt-BR', {
                                                                    year: 'numeric',
                                                                    month: '2-digit',
                                                                    day: '2-digit'
                                                                ...
                                                                <>
                                                                    ...
                                                                    {(group.isOwn || isOwner || isModerator) && (
                                                                        <button onClick={() => deleteMessage(message.id)} className="text-gray-400 hover:text-red-400">
                                                                            <i className="fas fa-trash" title={group.isOwn ? 'Deletar' : 'Deletar via moderador'}></i>
                                                                        </button>
                                                                    )}
                                                                </>
                                                            )}
                                        ...
                                        {!group.isOwn && (
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {group.senderProfile?.role === 1 && <span title="Moderador do MonChat" className="role-tag mod">Mod</span>}
                                            </div>
                                        )}
                                    </div>
                ...
                {isWrongNetwork && isConnected ? (
                    <div className="text-center p-4 bg-red-900/50 border-t-2 border-red-500 rounded-t-lg animate-pulse">
                        ...
                        <p className="text-red-200 mt-1">
                            Por favor, mude para a <strong>MONAD TESTNET</strong> para continuar. NÃO TENTE INTERAÇÕES ATÉ QUE A REDE SEJA ALTERADA.
                        </p>
                    </div>
                ) : isConnected ? (
                    <>
                            ...
                            <div className="dashboard flex items-center gap-2">
                                ...
                                <div className="flex-1">
                                    <textarea
                                        ...
                                        placeholder={editingMessage ? "Edite a mensagem..." : "Digite sua mensagem..."}
